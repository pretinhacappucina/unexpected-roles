...
internal static class CustomRolesHelper
{
    ...
    extension(CustomRoles role)
    {
        ...
        public RoleBase GetRoleClass()
        {
            RoleBase roleClass = role switch
            {
                ...
                _ => Main.AllRoleClasses.Find(x => x.GetType().Name.Equals(role.ToString(), StringComparison.OrdinalIgnoreCase)) ?? new VanillaRole()
            };
        ...
        public CustomRoles GetVNRole(bool checkDesyncRole = false)
        {
            ...
            return role switch
            {
                ...
                CustomRoles.Noisebodie => CustomRoles.Crewmate,
                ...
                CustomRoles.Tyrant => CustomRoles.Impostor,
                ...
                CustomRoles.Evader => CustomRoles.Crewmate,
                ...
                CustomRoles.Revelator => CustomRoles.Engineer,
                CustomRoles.BomberMan => CustomRoles.Phantom,
                CustomRoles.Whistleblower => CustomRoles.Engineer,
                CustomRoles.Worker => CustomRoles.Engineer,
                CustomRoles.Teleporter => CustomRoles.Shapeshifter,
                CustomRoles.Recruiter => CustomRoles.Shapeshifter,
        ...
        public RoleTypes GetDYRole(bool load = false)
        {
            ...
            return role switch
            {
                ...
                // DoomTag
                CustomRoles.Tagger => RoleTypes.Impostor,
                // Standard
        ...
        public bool IsImpostor()
        {
            return (role == CustomRoles.DoubleAgent && (!Options.IsLoaded || !Main.IntroDestroyed)) || role is
                ...
                CustomRoles.BomberMan or
                CustomRoles.Teleporter or
                CustomRoles.Recruiter or
        ...
        public RoleOptionType GetNeutralRoleCategory()
        {
            return role switch
            {
                ...
                CustomRoles.Noisebodie => RoleOptionType.Neutral_Evil,
        ...
        public RoleOptionType GetImpostorRoleCategory()
        {
            return role switch
            {
                ...
                CustomRoles.Tyrant => RoleOptionType.Impostor_Support,
                ...
                CustomRoles.BomberMan => RoleOptionType.Impostor_Concealing,
                CustomRoles.Teleporter => RoleOptionType.Impostor_Concealing,
                CustomRoles.Recruiter => RoleOptionType.Impostor_Concealing,
        ...
        public RoleOptionType GetCrewmateRoleCategory()
        {
            return role switch
            {
                ...
                CustomRoles.Evader => RoleOptionType.Crewmate_Miscellaneous,
    ...
    public static bool CheckAddonConflict(CustomRoles role, PlayerControl pc, bool validateExistingState = false)
    {
        return role.IsAdditionRole() && !(role.IsGhostRole() && pc.IsAlive()) && (!Main.NeverSpawnTogetherCombos.TryGetValue(OptionItem.CurrentPreset, out Dictionary<CustomRoles, List<CustomRoles>> neverList) || !neverList.TryGetValue(pc.GetCustomRole(), out List<CustomRoles> bannedAddonList) || !bannedAddonList.Contains(role)) && pc.GetCustomRole() is not CustomRoles.GuardianAngelEHR and not CustomRoles.God && !pc.Is(CustomRoles.Madmate) && !pc.Is(CustomRoles.GM) && (validateExistingState || role is not CustomRoles.Lovers) && !pc.Is(CustomRoles.LazyGuy) && (validateExistingState || (Main.IntroDestroyed && Options.AddonAssigningRolesIgnoreMaxAddonsLimit.GetBool()) || !pc.HasSubRole() || pc.GetCustomSubRoles().Count < Options.NoLimitAddonsNumMax.GetInt()) && (!Options.AddonCanBeSettings.TryGetValue(role, out (OptionItem Imp, OptionItem Neutral, OptionItem Crew, OptionItem Coven) o) || ((o.Imp.GetBool() || !pc.GetCustomRole().IsImpostor()) && (o.Neutral.GetBool() || !pc.GetCustomRole().IsNeutral()) && (o.Crew.GetBool() || !pc.IsCrewmate()) && (o.Coven.GetBool() || !pc.Is(Team.Coven)))) && (!role.IsImpOnlyAddon() || (pc.IsImpostor() && !pc.Is(CustomRoles.DoubleAgent)) || (pc.Is(CustomRoles.Traitor) && Traitor.CanGetImpostorOnlyAddons.GetBool())) && role switch
        {
            ...
            CustomRoles.Torch when pc.Is(CustomRoles.Bewilder) || pc.Is(CustomRoles.Sunglasses) || pc.Is(CustomRoles.GuardianAngelEHR) => false,
