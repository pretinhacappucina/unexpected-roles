...
internal static class MapBehaviourShowPatch
{
    public static bool Prefix(MapBehaviour __instance, ref MapOptions opts)
    {
        ...
        if (player.GetCustomRole() == CustomRoles.Hacker && opts.Mode is not MapOptions.Modes.CountOverlay && Hacker.PlayerIdList != null && Hacker.PlayerIdList.ContainsKey(player.PlayerId))
        {
            ...
            case CustomGameMode.HotPotato:
                {
                    FinalTextBuilder.Append("\r\n\r\n")
                        .Append("<size=90%>")
                        .Append(Main.CachedAllPlayerControls().Select(pc => new { pc, alive = pc.IsAlive() }).OrderByDescending(x => x.alive).Join('\n', x => $"{(!x.alive ? "<#777777><s>" : "")}{HotPotato.GetIndicator(x.pc.PlayerId)}{(x.alive ? x.pc.PlayerId.ColoredPlayerName() : Main.AllPlayerNames.GetValueOrDefault(x.pc.PlayerId, $"ID {x.pc.PlayerId}"))}{(!x.alive ? $"</color></s>  <#ff0000>{GetString("Dead")}</color>" : "</b>")}"))
                        .Append("</size>");
                    break;
                }
            case CustomGameMode.HideAndSeek:
